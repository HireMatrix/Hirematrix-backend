    ...
    while (pageCount < maxPages) {
        ...
        for (let i = 0; i < jobCards.length; i++) {
            ...
            try {
                ...
                const { jobTitle, jobCompany, location, salary, jobTags, jobDescription, jobDetails } = await parseJobFromPage(newPage);
                ...
                try {
                    const cachKey = `job:${jobTitle}:${jobCompany}`;
                    const cachedJob = await redisClient.get(cachKey);

                    if (cachedJob) {
                        AllJobs.push(JSON.parse(cachedJob)); 
                    } else {
                        const response = await axios.post('http://127.0.0.1:8000/parse-job', { prompt });
